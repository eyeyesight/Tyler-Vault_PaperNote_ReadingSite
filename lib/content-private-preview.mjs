...
const rendererInstallTimeoutMs = 120_000
...
const opaqueOperationPattern = /^content-[0-9a-f]{20}$/u
...
export function routeForSource(source) {
  ...
  throw new ContentPreviewError("DISCOVERY_TARGET_UNSUPPORTED", "new content must be a direct paper or approved Knowledge page")
}
...
/** @param {{version:number,operation_id:string,lane:"content",status:string,summary:string,added_routes:any[],changed_routes:any[],removed_routes:any[],checks:any[],next_action:string,error_code:string|null,mapping_identity:any,candidate_identity:any,preview:any}} result */
function assertStructured(result) {
  if (result.version !== 1 || !isOpaqueOperationId(result.operation_id) || result.lane !== "content" || !statuses.has(result.status)
 ...
 * @param {{removeOwnedDirectory?:(claim:{path:string,canonical:string,identity:import("node:fs").Stats},ownerCanonical?:string)=>Promise<boolean>}} [internal]
 */
async function prepareContentPrivatePreviewController(options, internal = {}) {
  ...
  let rendererClaim = null
  let rendererRoot = ""
  let rendererOwned = false
  let rendererTreeSha256 = null
  ...
  try {
    ...
    outputRoot = path.join(sessionRoot, "built-site")
    ...
    rendererClaim = await createRendererDirectory(cleanupOwnedDirectory)
    rendererOwned = true
    if (pathsOverlap(vaultRoot, rendererClaim.canonical) || pathsOverlap(workspaceCanonical, rendererClaim.canonical)) {
      throw new ContentPreviewError("PATH_OVERLAP_NOT_ALLOWED", "live renderer materialization must be disjoint from private inputs")
    }
    const materializedRenderer = await materializeGitRenderer(authority.gitOptions, authority.liveRendererSha, rendererClaim)
    rendererRoot = materializedRenderer.root
    rendererTreeSha256 = materializedRenderer.treeSha256
    const buildOptions = { ...slimOptions, command: "build", rendererRoot }
    ...
    const candidate = await candidateIdentity(path.join(handoffRoot, "site"))
    const candidateBinding = sha256(Buffer.from(`source_main=${authority.mainSha}\ngh_pages=${authority.ghPagesSha}\nmap=${mappingIdentity.map_sha256}\nlive_renderer=${authority.liveRendererSha}\nrenderer_tree=${rendererTreeSha256}\nsite=${candidate.digest}\n`, "utf8"))
    const changed = addedRoutes.length > 0 || changedRoutes.length > 0 || handoff.diff.added.length > 0 || handoff.diff.deleted.length > 0 || handoff.diff.changed.length > 0
    if (!changed) {
      keepSession = false
    ...
    result = assertStructured({
      ...
      candidate_identity: {
        ...
        renderer_tree_sha256: rendererTreeSha256,
    ...
    if (rendererOwned && rendererClaim) await cleanupOwnedDirectory(rendererClaim)
