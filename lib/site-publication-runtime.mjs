...
function exactNoChangeIdentity(publication, operation) {
  const convergence = publication?.convergence
  if (!isPlainObject(convergence)
    || JSON.stringify(Object.keys(convergence).sort()) !== JSON.stringify([
      "desired_site_sha256",
      "exact",
      "live_site_sha256",
      "provider_site_commit",
      "public_site_sha256",
    ])
    || convergence.exact !== true) fail("PUBLICATION_RESULT_INVALID")
  const desired = boundedSha(convergence.desired_site_sha256, 64)
  const publicSite = boundedSha(convergence.public_site_sha256, 64)
  const live = boundedSha(convergence.live_site_sha256, 64)
  const operationSite = boundedSha(operation?.candidate_identity?.site_sha256, 64)
  const siteCommit = boundedSha(convergence.provider_site_commit, 40)
  if (!desired || desired !== publicSite || desired !== live || desired !== operationSite || !siteCommit) fail("PUBLICATION_RESULT_INVALID")
  return { siteCommit, siteSha: desired }
}
...
export async function runSitePublication(operation, settings = {}, dependencies = {}) {
  ...
  let currentLkg
  let currentLkgRead = false
  if (safePublication.status === "no_change") {
    try {
      const identity = exactNoChangeIdentity(publication, operation)
      currentLkg = await lkg.readCurrent()
      currentLkgRead = true
      if (currentLkg !== null) currentLkg = validateLkgRecord(currentLkg)
      if (currentLkg?.site_commit === identity.siteCommit && currentLkg?.site_sha256 === identity.siteSha) return safePublication
      safePublication = {
        ...safePublication,
        status: "published",
        identifiers: { ...safePublication.identifiers, site_commit: identity.siteCommit },
        site_sha256: identity.siteSha,
      }
    } catch {
      return resultWithFailure(safePublication, operation, "PUBLICATION_RESULT_INVALID")
    }
  }
  ...
  if (liveQa.status === "pass") {
    let record
    try {
      record = await lkg.record(lkgCandidateRecord(safePublication, operation, liveQa))
