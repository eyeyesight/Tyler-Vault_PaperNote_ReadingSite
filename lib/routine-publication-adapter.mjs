...
const BRANCH = /^publication\/map\/[a-z0-9][a-z0-9._-]{0,79}$/u
...
function commitMessage(rendererSha) {
  return Buffer.from(`Publication gh-pages candidate\n\nRenderer-Main-SHA: ${rendererSha}\n`, "utf8")
}
...
async function makeOperationDirectory(config) {
  try {
    ...
    const directory = await mkdtemp(path.join(config.operationRoot, ".pub-git-"))
...
async function withIndex(config, transport, callback) {
  ...
  const env = {
    ...
    GIT_AUTHOR_NAME: "Tyler-Vault routine publication",
    GIT_AUTHOR_EMAIL: "routine-publication@invalid",
    GIT_COMMITTER_NAME: "Tyler-Vault routine publication",
    GIT_COMMITTER_EMAIL: "routine-publication@invalid",
  }
...
async function createMappingCommit(config, transport, input) {
  ...
  return await withIndex(config, transport, async (_operation, env) => {
    ...
    const message = Buffer.from(`Publication mapping proposal ${input.branch}\n`, "utf8")
...
async function pushCandidate(config, transport, input) {
  ...
  try {
    await runGit(config, transport, [
      "push",
      "--porcelain",
      `--force-with-lease=${config.ghPagesRef}:${input.expected_old_sha}`,
      config.remote,
      `${input.candidate_sha}:${config.ghPagesRef}`,
    ])
  } catch (error) {
    pushError = error
  }
...
const PROVIDER_MAPPING_WORKFLOW = "ci.yml"
const PROVIDER_MAPPING_JOB = "CI"
...
function providerDependencies(value) {
  ...
  if (raw.httpTransport === undefined) {
    const transport = createBoundedHttpTransport()
    return { command, http: transport.get.bind(transport) }
  }
...
export const _testOnlyProviderDependencies = providerDependencies
...
async function anonymousSmoke(config, http, input) {
  ...
  if (notFound.expected_status !== 404 || notFound.path !== "/__publication_missing__") throw fail("smoke_failed")
