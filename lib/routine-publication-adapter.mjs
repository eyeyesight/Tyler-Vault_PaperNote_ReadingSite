...
function validateConfig(value) {
  const raw = exactRecord(value, ["gitExecutable", "gitRoot", "remote", "mainRef", "ghPagesRef", "operationRoot"], "config_invalid")
...
async function createCandidateCommit(config, transport, input) {
  ...
  const candidate = await verifyDirectory(input.candidate_path, {
    canonical: config.operationRootCanonical,
...
function providerCommandRequest(config, args, { input, mutation = false } = {}) {
  const request = {
    argv: Object.freeze([config.ghExecutable, "api", "--repo", config.repository, ...args]),
    cwd: config.ghCwd,
