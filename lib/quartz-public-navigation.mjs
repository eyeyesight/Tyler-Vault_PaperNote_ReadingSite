import { createPublicGraphExplorerScript } from "./public-graph-explorer.mjs"
...
function normalizeEntries(entries) {
  ...
  const normalized = entries.map((entry) => {
    ...
    const label = ["concept", "method", "task"].includes(nodeClass) ? uppercaseInitial(rawLabel) : rawLabel
...
function graphMarkup(currentPublicId) {
  ...
  const graphTypeIcons = {
    paper: { id: "tabler-file-text", body: '<path d="M14 3v4a1 1 0 0 0 1 1h4"/><path d="M17 21H7a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h7l5 5v11a2 2 0 0 1-2 2z"/><path d="M9 13h6"/><path d="M9 17h6"/>' },
    concept: { id: "lucide-network", body: '<rect x="16" y="16" width="6" height="6" rx="1"/><rect x="2" y="16" width="6" height="6" rx="1"/><rect x="9" y="2" width="6" height="6" rx="1"/><path d="M5 16v-3a1 1 0 0 1 1-1h12a1 1 0 0 1 1 1v3"/><path d="M12 12V8"/>' },
    method: { id: "lucide-flask-conical", body: '<path d="M14 2v6a2 2 0 0 0 .245.96l5.51 10.08A2 2 0 0 1 18 22H6a2 2 0 0 1-1.755-2.96l5.51-10.08A2 2 0 0 0 10 8V2"/><path d="M6.453 15h11.094"/><path d="M8.5 2h7"/>' },
    task: { id: "lucide-list-checks", body: '<path d="M13 5h8"/><path d="M13 12h8"/><path d="M13 19h8"/><path d="m3 17 2 2 4-4"/><path d="m3 7 2 2 4-4"/>' },
    author: { id: "lucide-users-round", body: '<path d="M18 21a8 8 0 0 0-16 0"/><circle cx="10" cy="8" r="5"/><path d="M22 20c0-3.37-2-6.5-4-8a5 5 0 0 0-.45-8.3"/>' },
  }
  /** @param {{id:string,body:string}} graphic @param {string} [className] */
  const icon = (graphic, className = "public-graph-filter-icon") => `<svg class="${className}" data-icon="${graphic.id}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false">${graphic.body}</svg>`
  const filters = classGroups.slice(0, 5).map(({ nodeClass, label }) => `
    <button type="button" data-graph-filter="${nodeClass}" aria-pressed="true">
      <span class="public-graph-swatch" data-node-class="${nodeClass}" aria-hidden="true"></span>
      ${icon(graphTypeIcons[nodeClass])}
      <span data-graph-filter-label>${label}</span>
      <span data-graph-filter-count></span>
    </button>`).join("")
  const graphTitle = scope === "global" ? "Global Graph" : "Local Graph"
  const openNoteIcon = icon(graphTypeIcons.paper, "public-graph-action-icon")
  const sourceIcon = icon({ id: "lucide-external-link", body: '<path d="M15 3h6v6"/><path d="M10 14 21 3"/><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/>' }, "public-graph-action-icon")

  return `<section class="public-graph" id="${escapeHtml(graphId)}" data-graph-scope="${scope}"${root} data-layout-ready="false" data-graph-layout="phone" data-graph-zoom="mid" data-sheet-state="peek">
