...
/**
 * Resolve and read one runtime-owned contract without following links at any
 * contract-controlled layer. Absolute runtime roots are context, never contract
 * paths; only the normalized relative receipt path is traversed here.
 * @param {{canonicalRoot:string}} opened
 * @param {string} relativePath
 * @param {string} missingCode
 */
async function readRuntimeContract(opened, relativePath, missingCode) {
  ...
  for (let index = 0; index < segments.length; index += 1) {
    ...
    try {
      ...
      throw new ContractError("RUNTIME_READ_FAILED", "runtime contract directory could not be read")
    }
    ...
    try {
      ...
      throw new ContractError("RUNTIME_READ_FAILED", "runtime contract metadata could not be read")
    }
    if (metadata.isSymbolicLink()) throw new ContractError("PATH_SYMLINK_NOT_ALLOWED", "runtime contract path cannot contain a symlink, junction, or reparse point")
    const final = index === segments.length - 1
    if (final ? !metadata.isFile() : !metadata.isDirectory()) {
      throw new ContractError("RUNTIME_FILE_CLASS_INVALID", "runtime contract path must end in a regular file beneath regular directories")
    }
    let resolved
    try {
      resolved = await realpath(candidate)
    } catch {
      throw new ContractError("RUNTIME_READ_FAILED", "runtime contract path could not be resolved")
    }
    if (!inside(opened.canonicalRoot, resolved)) throw new ContractError("PATH_CONTAINMENT_ESCAPE", "resolved runtime contract escapes its root")
    directory = resolved
  }
  return readContractJson(directory)
}
...
async function requireGenesisHistoryAbsent(opened) {
  let names
  try {
    names = await readdir(opened.canonicalRoot)
  } catch {
    throw new ContractError("RUNTIME_READ_FAILED", "runtime root could not be read while checking genesis history")
  }
  rejectDirectoryCaseCollisions(names)
  if (!await hasExactRuntimeSegment(opened.canonicalRoot, names, "consumed")) {
    return
  }

  const consumedPath = path.join(opened.canonicalRoot, "consumed")
  let metadata
  try {
    metadata = await lstat(consumedPath)
  } catch {
    throw new ContractError("RUNTIME_READ_FAILED", "runtime consumed history metadata could not be read")
  }
  if (metadata.isSymbolicLink()) throw new ContractError("PATH_SYMLINK_NOT_ALLOWED", "runtime consumed history cannot be a symlink, junction, or reparse point")
  if (!metadata.isDirectory()) throw new ContractError("GENESIS_HISTORY_PRESENT", "genesis is forbidden when a consumed history entry exists")

  let canonicalConsumed
  let entries
  try {
    canonicalConsumed = await realpath(consumedPath)
    if (!inside(opened.canonicalRoot, canonicalConsumed)) throw new ContractError("PATH_CONTAINMENT_ESCAPE", "resolved runtime consumed history escapes its root")
    entries = await readdir(canonicalConsumed)
  } catch (error) {
    if (error instanceof ContractError) throw error
    throw new ContractError("RUNTIME_READ_FAILED", "runtime consumed history could not be read")
  }
...
/** Validate a pointer and load the exact receipt it names. @param {{canonicalRoot:string}} opened @param {any} pointer */
async function loadPointedReceipt(opened, pointer) {
  await validateContract("current-release", pointer)
  const receipt = /** @type {any} */ (await readRuntimeContract(opened, pointer.receipt_path, "CURRENT_RECEIPT_MISSING"))
...
/**
 * Read-only loader for the fixed runtime current pointer and its consumed receipt.
 * Genesis requires a genuinely absent, correctly-cased pointer and no possible
 * consumed receipt history.
...
export async function loadPublicationRuntime(runtimeRoot) {
  ...
  let pointer
  try {
    pointer = await readRuntimeContract(opened, "current-release.json", "CURRENT_POINTER_ABSENT")
  } catch (error) {
    if (error instanceof ContractError && error.code === "CURRENT_POINTER_ABSENT") {
      await requireGenesisHistoryAbsent(opened)
      return { currentPointer: undefined, currentReceipt: undefined, receiptPath: undefined }
    }
    throw error
  }
  await validateContract("current-release", pointer)
  const receipt = await loadPointedReceipt(opened, pointer)
  return { currentPointer: pointer, currentReceipt: receipt, receiptPath: pointer.receipt_path }
}
...
/** Validate a candidate pointer by resolving its named receipt inside runtime root. @param {any} pointer @param {{runtimeRoot:string}} options */
export async function validateCurrentReleaseCandidate(pointer, options) {
  ...
  const opened = await openRuntimeRoot(options.runtimeRoot)
  const receipt = await loadPointedReceipt(opened, pointer)
  return { kind: "current-release", schemaVersion: pointer.schema_version, value: pointer, receipt }
}
