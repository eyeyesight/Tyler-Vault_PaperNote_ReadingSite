...
const typedArrayIntrinsicPrototype = Object.getPrototypeOf(Uint8Array.prototype)
const typedArrayByteLengthGetter = Object.getOwnPropertyDescriptor(typedArrayIntrinsicPrototype, "byteLength")?.get
const typedArrayBufferGetter = Object.getOwnPropertyDescriptor(typedArrayIntrinsicPrototype, "buffer")?.get
const uint8ArrayAt = Uint8Array.prototype.at
...
const maxContractInputBytes = 16 * 1024 * 1024
const maxJsonDepth = 128
const maxJsonNodes = 100000
const maxJsonContainerMembers = 4096
const maxJsonStringBytes = 1024 * 1024
const maxJsonTotalStringBytes = 8 * 1024 * 1024
const maxJsonNumberTokenChars = 128
const schemaFiles = {
 ...
 * Fixed budgets are checked before recursive descent, member insertion, and
 * decoded-string growth; the depth bound keeps this parser away from stack
 * overflow while preserving its deliberately small implementation.
 * @param {string} text
 */
function parseIJson(text) {
  let index = 0
  const budget = { totalNodes: 0, totalStringBytes: 0 }
  /** @returns {never} */
  const syntax = () => { throw new SyntaxError(`invalid JSON at offset ${index}`) }
  /** @returns {never} */
  const resourceLimit = () => { throw new ContractError("INPUT_RESOURCE_LIMIT", "contract JSON exceeds the fixed resource budget") }
  /** @param {number} depth */
  const consumeNode = (depth) => {
    if (depth > maxJsonDepth || budget.totalNodes >= maxJsonNodes) resourceLimit()
    budget.totalNodes += 1
  }
  ...
  /** @param {string} part @param {{bytes:number,pendingHigh:boolean}} state */
  const appendStringPart = (part, state) => {
    for (let offset = 0; offset < part.length; offset += 1) {
      const codeUnit = part.charCodeAt(offset)
      let addedBytes = 0
      let paired = false
      if (state.pendingHigh) {
        if (codeUnit >= 0xdc00 && codeUnit <= 0xdfff) {
          addedBytes = 4
          state.pendingHigh = false
          paired = true
        } else {
          addedBytes = 3
          state.pendingHigh = false
      ...
      if (!paired) {
        if (codeUnit >= 0xd800 && codeUnit <= 0xdbff) state.pendingHigh = true
        else if (codeUnit >= 0xdc00 && codeUnit <= 0xdfff) addedBytes += 3
        else if (codeUnit <= 0x7f) addedBytes += 1
        else if (codeUnit <= 0x7ff) addedBytes += 2
        else addedBytes += 3
      }
      if (state.bytes + addedBytes > maxJsonStringBytes
        || budget.totalStringBytes + state.bytes + addedBytes > maxJsonTotalStringBytes) resourceLimit()
  ...
  const parseString = () => {
    ...
    index += 1
    let decoded = ""
    const state = { bytes: 0, pendingHigh: false }
    while (index < text.length) {
      ...
      if (character === '"') {
        index += 1
        if (state.pendingHigh) {
          if (state.bytes + 3 > maxJsonStringBytes
            || budget.totalStringBytes + state.bytes + 3 > maxJsonTotalStringBytes) resourceLimit()
          state.bytes += 3
        }
        const value = scalarString(decoded)
        budget.totalStringBytes += state.bytes
        return value
      }
      if (codeUnit <= 0x1f) syntax()
      if (character !== "\\") {
        decoded += appendStringPart(character, state)
        index += 1
        continue
      }
      index += 1
      ...
      if (escape === "u") {
        ...
        decoded += appendStringPart(String.fromCharCode(Number.parseInt(hexadecimal, 16)), state)
        index += 5
        continue
      }
      const escapedCharacters = { '"': '"', "\\": "\\", "/": "/", b: "\b", f: "\f", n: "\n", r: "\r", t: "\t" }
      if (!escape || !Object.hasOwn(escapedCharacters, escape)) syntax()
      decoded += appendStringPart(escapedCharacters[/** @type {keyof typeof escapedCharacters} */ (escape)], state)
      index += 1
    }
    syntax()
  }
  const parseNumber = () => {
    ...
    while (index < text.length && /[0-9eE+.-]/.test(text[index])) {
      if (index - start >= maxJsonNumberTokenChars) resourceLimit()
      index += 1
    }
    const token = text.slice(start, index)
    if (!/^-?(?:0|[1-9][0-9]*)(?:\.[0-9]+)?(?:[eE][+-]?[0-9]+)?$/.test(token)) syntax()
  ...
  const parseArray = (depth) => {
    ...
    while (true) {
      if (memberCount >= maxJsonContainerMembers) resourceLimit()
      memberCount += 1
...
export function decodeContractJsonBytes(bytes) {
  if (utilTypes.isProxy(bytes)) {
    throw new ContractError("INPUT_BYTES_INVALID", "contract input bytes must be an ordinary Buffer")
  }
  let byteLength
  try {
    if (!Reflect.apply(bufferIsBuffer, Buffer, [bytes]) || Object.getPrototypeOf(bytes) !== bufferPrototype
      || typeof typedArrayByteLengthGetter !== "function") {
      throw new Error("not an exact Buffer")
    }
    byteLength = Reflect.apply(typedArrayByteLengthGetter, bytes, [])
  } catch {
    throw new ContractError("INPUT_BYTES_INVALID", "contract input bytes must be an ordinary Buffer")
  }
  if (!Number.isSafeInteger(byteLength) || byteLength < 0) {
    throw new ContractError("INPUT_BYTES_INVALID", "contract input bytes must have a stable intrinsic length")
  }
  if (byteLength > maxContractInputBytes) {
    throw new ContractError("INPUT_RESOURCE_LIMIT", "contract input exceeds the fixed byte budget")
  }
  let first
  let second
  let third
  let backingBuffer
  try {
    if (typeof uint8ArrayAt !== "function" || typeof typedArrayBufferGetter !== "function") throw new Error("missing typed-array intrinsic")
    first = Reflect.apply(uint8ArrayAt, bytes, [0])
    second = Reflect.apply(uint8ArrayAt, bytes, [1])
    third = Reflect.apply(uint8ArrayAt, bytes, [2])
    backingBuffer = Reflect.apply(typedArrayBufferGetter, bytes, [])
    if (backingBuffer === null || typeof backingBuffer !== "object" || Object.getPrototypeOf(backingBuffer) !== arrayBufferPrototype
      || typeof arrayBufferByteLengthGetter !== "function") throw new Error("invalid backing buffer")
    Reflect.apply(arrayBufferByteLengthGetter, backingBuffer, [])
    if (typeof arrayBufferResizableGetter === "function" && Reflect.apply(arrayBufferResizableGetter, backingBuffer, [])) {
      throw new Error("resizable backing buffer")
    }
  } catch {
    throw new ContractError("INPUT_BYTES_INVALID", "contract input bytes must have stable ordinary backing storage")
  }
  if (first === 0xef && second === 0xbb && third === 0xbf) {
    throw new ContractError("INPUT_BOM_NOT_ALLOWED", "contract JSON must not contain a UTF-8 BOM")
  }
  let text
  try {
