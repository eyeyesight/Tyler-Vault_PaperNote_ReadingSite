...
import { parseZoteroManagedBlock } from "./zotero-delta.mjs"
 ...
 * standalone-valid receipt named exactly by the approved release baseline.
 * Initial metadata for a newly published primary paper is derived here from
 * exact source bytes; callers cannot supply digest-shaped metadata directly.
 */
/** @param {{manifest:any,currentReceipt?:any,currentReceiptPath?:string,createdAt:string,projectedMarkdown:Map<string,Buffer>,artifacts:Artifact[],zoteroSourceBytes?:Map<string,Buffer>,zoteroBaselines?:Map<string,any>}} input */
export async function constructReleaseReceipt({ manifest, currentReceipt, currentReceiptPath, createdAt, projectedMarkdown, artifacts, zoteroSourceBytes = new Map(), zoteroBaselines }) {
  if (zoteroBaselines !== undefined) {
    throw new ContractError("ZOTERO_BASELINE_INPUT_INVALID", "callers cannot supply Zotero baseline hash metadata")
  }
  if (!(zoteroSourceBytes instanceof Map)) {
    throw new ContractError("ZOTERO_SOURCE_BYTES_INVALID", "Zotero source input must be keyed by public ID")
  }
  ...
  const allowedNewPaperId = manifest.action.kind === "publish-unit"
    && manifest.action.added_node_ids.includes(manifest.action.primary_id)
    && !currentById.has(manifest.action.primary_id)
    ? manifest.action.primary_id
    : undefined
  /** @type {Map<string,any>} */
  const derivedBaselines = new Map()
  for (const [publicId, sourceBytes] of zoteroSourceBytes) {
    if (publicId !== allowedNewPaperId) {
      throw new ContractError("ZOTERO_SOURCE_TARGET_INVALID", "Zotero source bytes are allowed only for the newly published primary paper")
    }
    if (!Buffer.isBuffer(sourceBytes)) {
      throw new ContractError("ZOTERO_SOURCE_BYTES_INVALID", "Zotero source input must contain exact bytes")
    }
    const node = manifest.nodes.find((/** @type {any} */ candidate) => candidate.public_id === publicId)
    if (!node || node.node_class !== "paper") {
      throw new ContractError("ZOTERO_SOURCE_TARGET_INVALID", "Zotero source bytes are allowed only for the newly published primary paper")
    }
    if (sha256(sourceBytes) !== node.source_sha256) {
      throw new ContractError("ZOTERO_SOURCE_HASH_MISMATCH", "Zotero source bytes do not match the approved manifest hash")
    }
    derivedBaselines.set(publicId, parseZoteroManagedBlock(sourceBytes).metadata)
  }
  const nodes = manifest.nodes.map((/** @type {any} */ node) => {
    ...
    if (derivedBaselines.has(node.public_id)) {
      releaseNode.zotero_baseline = structuredClone(derivedBaselines.get(node.public_id))
    } else if (baseline && Object.hasOwn(baseline, "zotero_baseline")) {
...
function sameCanonicalPathSpelling(requested, canonical) {
  return requested === canonical || (process.platform === "win32" && asciiCaseFold(requested) === asciiCaseFold(canonical))
}
...
async function openCandidateRoot(root) {
  ...
  try {
    ...
    await assertNoLinkAncestors(absolute, {
      errorFactory: () => new ContractError("CANDIDATE_ARTIFACT_ROOT_INVALID", "candidate artifact root is not a stable canonical ordinary directory"),
    })
  ...
  if (!sameCanonicalPathSpelling(absolute, canonical) || !sameDirectoryMetadata(before, after)) {
...
export async function readCandidateArtifactTree(root) {
  ...
  async function walk(directory, prefix) {
    ...
    if (!sameDirectoryMetadata(before, after)) {
