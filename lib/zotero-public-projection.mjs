...
import { SlimContentError } from "./slim-content-map.mjs"
...
function nodeOffsets(node) {
  ...
  if (!Number.isInteger(start) || !Number.isInteger(end) || start < 0 || end < start) {
    throw new SlimContentError("SOURCE_MARKDOWN_INVALID", "Zotero Markdown nodes require stable source offsets")
  }
  return { start, end }
}
...
export function validateZoteroFrontmatter(frontmatter, nodeClass) {
  if (Object.hasOwn(frontmatter, "zotero_uri")) {
    ...
    if (nodeClass !== "paper" || typeof value !== "string" || !exactPrivateSelectUri.test(value)) {
      throw new SlimContentError("SOURCE_UNSAFE_URL_SCHEME", "Zotero frontmatter contains an unsupported local URL")
    }
  }
  for (const [key, value] of Object.entries(frontmatter)) {
    ...
    if (values.some((candidate) => containsZoteroSchemeDisclosure(candidate))) {
      throw new SlimContentError("SOURCE_UNSAFE_URL_SCHEME", "Zotero local URLs require the authenticated managed block")
    }
  }
}
...
export function zoteroManagedRange(markdown, tree) {
  ...
  if (markerMentions.length !== 2 || markers.length !== 2 || markers[0].value !== startMarker || markers[1].value !== endMarker) {
    throw new SlimContentError("SOURCE_MARKDOWN_INVALID", "Zotero managed markers must be one exact ordered root-level pair")
  }
  ...
  if (markdown.slice(start.start, start.end) !== startMarker || markdown.slice(end.start, end.end) !== endMarker || start.end > end.start) {
    throw new SlimContentError("SOURCE_MARKDOWN_INVALID", "Zotero managed marker offsets are invalid")
  }
...
function sanitizeManagedSlice(markdown) {
  ...
  if (containsZoteroSchemeDisclosure(safe)) {
    throw new SlimContentError("SOURCE_UNSAFE_URL_SCHEME", "Zotero managed content contains an unsupported local URL")
  }
  if (/<!--\s*zotero-annotation\b|`annotation:|Metadata:\s*attachment\s*`/i.test(safe)) {
    throw new SlimContentError("SOURCE_ACTIVE_CONTENT_NOT_ALLOWED", "Zotero managed content contains unsupported annotation metadata")
  }
  if (unsupportedManagedRawHtml.test(safe)) {
    throw new SlimContentError("SOURCE_ACTIVE_CONTENT_NOT_ALLOWED", "Zotero managed content contains unsupported raw HTML")
  }
  return safe
}
...
function parseMarkdown(markdown) {
  try {
    ...
    if (error instanceof SlimContentError) throw error
    throw new SlimContentError("SOURCE_MARKDOWN_INVALID", "source Markdown could not be parsed for Zotero projection")
  }
}
...
function markdownUrlNodes(markdown, tree) {
  ...
  /** @param {any} node */
  function walk(node) {
    if (!node || typeof node.type !== "string") throw new SlimContentError("SOURCE_MARKDOWN_INVALID", "Zotero Markdown tree is invalid")
    ...
    if (node.children !== undefined) {
      if (!Array.isArray(node.children)) throw new SlimContentError("SOURCE_MARKDOWN_INVALID", "Zotero Markdown children are invalid")
  ...
  return [...linkNodes, ...definitionNodes].map((node) => {
    ...
    if (end > markdown.length) throw new SlimContentError("SOURCE_MARKDOWN_INVALID", "Zotero Markdown offset escaped source")
    ...
    if (typeof resolved !== "string") throw new SlimContentError("SOURCE_MARKDOWN_INVALID", "Zotero Markdown link reference is unresolved")
