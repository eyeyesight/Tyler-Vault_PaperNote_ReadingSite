...
/** @returns {{entryFile:string,custom404:string}} */
function activeStaticSiteFiles() {
  const configured = deploymentContract?.site
  const entryFile = configured?.entry_file
  const custom404 = configured?.custom_404
  if (typeof entryFile !== "string" || entryFile.length === 0
    || typeof custom404 !== "string" || custom404.length === 0
    || entryFile === custom404) {
    throw new ContractError("CANDIDATE_STATIC_SITE_CONTRACT_INVALID", "active deployment contract static-site files are invalid")
  }
  try {
    validateRelativeCandidatePath(entryFile)
    validateRelativeCandidatePath(custom404)
  } catch {
    throw new ContractError("CANDIDATE_STATIC_SITE_CONTRACT_INVALID", "active deployment contract static-site files are invalid")
  }
  return { entryFile, custom404 }
}

/** @param {{path:string}[]} inventory @param {string} label */
function assertRequiredStaticSiteFiles(inventory, label) {
  const { entryFile, custom404 } = activeStaticSiteFiles()
  const paths = new Set(inventory.map((entry) => entry.path))
  for (const requiredPath of [entryFile, custom404]) {
    if (!paths.has(requiredPath)) {
      throw new ContractError("CANDIDATE_STATIC_SITE_FILE_MISSING", `${label} inventory is missing an active static-site file`)
    }
  }
}
...
/** @param {string} root */
async function validateGitControlEntry(root) {
  const absolute = path.join(root, gitControlName)
  let before
  try {
    await assertNoLinkAncestors(absolute, {
      errorFactory: () => new ContractError("CANDIDATE_SYMLINK_NOT_ALLOWED", "candidate .git control entry cannot be a symlink or reparse point"),
    })
    before = await lstat(absolute, { bigint: true })
    if (before.isSymbolicLink()) throw new ContractError("CANDIDATE_SYMLINK_NOT_ALLOWED", "candidate .git control entry cannot be a symlink or reparse point")
    if (!before.isFile() && !before.isDirectory()) throw new ContractError("CANDIDATE_TREE_SET_MISMATCH", "candidate .git control entry must be an ordinary file or directory")
    const canonical = await realpath(absolute)
    const after = await lstat(canonical, { bigint: true })
    const stable = before.isFile() ? sameFileMetadata(before, after) : sameDirectoryMetadata(before, after)
    if (!sameCanonicalPathSpelling(absolute, canonical) || !stable) {
      throw new ContractError("CANDIDATE_SYMLINK_NOT_ALLOWED", "candidate .git control entry cannot be a symlink or reparse point")
    }
  } catch (error) {
    if (error instanceof ContractError) throw error
    throw new ContractError("CANDIDATE_TREE_SET_MISMATCH", "candidate .git control entry must be an ordinary file or directory")
  }
}
...
function verifiedSourceSnapshot(materialized) {
  ...
  assertRequiredStaticSiteFiles(inventory, "sealed artifact")
...
function validateMetadataSemantics(metadata) {
  ...
  assertRequiredStaticSiteFiles(sourceInventory, "sealed artifact")
  assertRequiredStaticSiteFiles(siteInventory, "candidate site")
...
async function verifyCandidateInternal(candidateRoot, options = {}) {
  ...
  const hasGitControl = rootNames.includes(gitControlName)
  if (rootNames.length !== 2 + Number(hasGitControl)
    || !rootNames.includes(siteName)
    || !rootNames.includes(publicationName)
    || rootNames.some((name) => ![siteName, publicationName, gitControlName].includes(name))) {
    throw new ContractError("CANDIDATE_TREE_SET_MISMATCH", "candidate root must contain exactly site and .publication")
  }
  if (hasGitControl) await validateGitControlEntry(root)
  ...
  assertRequiredStaticSiteFiles(actualSiteInventory, "candidate site")
...
async function verifyStageExact(stageRoot, expected) {
  ...
  assertRequiredStaticSiteFiles(actual, "candidate staging")
