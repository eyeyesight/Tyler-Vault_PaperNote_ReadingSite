 ...
 * @property {unknown | null} public_projection
...
export function hashPublicProjection(projection) {
  const bytes = asBytes(projection)
  return sha256(bytes ?? canonicalJson(projection))
}
...
function validateProjection(candidate, operation, normalizedFiles) {
  const projection = candidate.public_projection ?? candidate.public_projection_bytes
  if (projection === undefined) return /** @type {string | null} */ (candidate.public_projection_sha256 ?? null)
  let value = projection
  let bytes = asBytes(projection)
  if (bytes) {
    try { value = JSON.parse(bytes.toString("utf8")) } catch { throw new PublicationControllerError("PRIVACY_FAILED", "verified") }
  }
  if (!isRecord(value)) throw new PublicationControllerError("PRIVACY_FAILED", "verified")
  for (const key of Object.keys(value)) {
    if (!PUBLIC_PROJECTION_KEYS.has(key)) throw new PublicationControllerError("PRIVACY_FAILED", "verified")
  }
  if (value.operation_id !== undefined && value.operation_id !== operation.operation_id) throw new PublicationControllerError("PRIVACY_FAILED", "verified")
  if (value.site_sha256 !== undefined && value.site_sha256 !== candidate.site_sha256 && value.site_sha256 !== candidate.desired_site_sha256) throw new PublicationControllerError("PRIVACY_FAILED", "verified")
  if (!bytes) bytes = Buffer.from(canonicalJson(value), "utf8")
  const digest = sha256(bytes)
  if (candidate.public_projection_sha256 !== undefined && candidate.public_projection_sha256 !== digest) throw new PublicationControllerError("CANDIDATE_INVALID", "verified")
...
function normalizeCandidate(candidate, operation) {
  ...
  const projectionSha256 = validateProjection(candidate, operation, files)
  if (projectionSha256 !== null && !validSha(projectionSha256, "sha256")) throw new PublicationControllerError("CANDIDATE_INVALID", "verified")
  return {
    files,
    routes: [...routes],
    ...
    public_projection_sha256: projectionSha256,
    public_projection: candidate.public_projection ?? candidate.public_projection_bytes ?? null,
 ...
 */
function exactDispatchInput(operation, frozen, candidate, siteCommit) {
  return {
    operation_id: operation.operation_id,
    source_main_sha: frozen.source_main_sha,
    vault_export_sha256: frozen.vault_export_sha256,
    ...
    public_projection_sha256: candidate.public_projection_sha256,
  }
}
 ...
 */
async function dispatchOnce(dependencies, operation, frozen, candidate, siteCommit) {
  const input = exactDispatchInput(operation, frozen, candidate, siteCommit)
...
async function runPublication(rawOperation, settings, dependencies) {
  ...
  const initialDeployment = await readLiveIdentity(
    ...
    candidate,
    initial,
    { ...exactDispatchInput(operation, frozen, candidate, frozen.expected_gh_pages_sha), site_commit: initial.provider_site_commit ?? frozen.expected_gh_pages_sha },
    "verifying_live",
  )
  ...
  const dispatch = await dispatchOnce(dependencies, operation, frozen, candidate, siteCommit)
  ...
  result.public_projection_sha256 = candidate.public_projection_sha256
  return result
}
