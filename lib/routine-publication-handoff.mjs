...
const MAPPING_BRANCH_PREFIX = "publication/map/"
const MAPPING_WORKFLOW = "ci.yml"
const MAPPING_JOB = "CI"
...
const SMOKE_SETTLE_ATTEMPTS = 24
...
function providerCode(stage, error) {
  ...
  if (stage === "mapping_push") return "push_uncertain"
...
function liveSmokeState(smoke, deploymentTarget, smokeInput) {
  if (!isRecord(smoke) || !sameDeploymentTarget(smoke.target, deploymentTarget)
    || !Array.isArray(smoke.route_statuses) || smoke.route_statuses.length !== smokeInput.routes.length
    || !Array.isArray(smoke.asset_statuses) || smoke.asset_statuses.length !== smokeInput.assets.length) return "failed"
  const publicStatuses = [...smoke.route_statuses, ...smoke.asset_statuses]
  if (smoke.homepage_status === 200 && smoke.not_found_status === 404
    && publicStatuses.every((value) => value === 200)) return "ready"
  if (smoke.homepage_status === 200 && smoke.not_found_status === 404
    && publicStatuses.some((value) => value === 404)
    && publicStatuses.every((value) => value === 200 || value === 404)) return "settling"
  return "failed"
}

async function waitForLiveSmoke(read, deploymentTarget, smokeInput) {
  for (let attempt = 1; attempt <= SMOKE_SETTLE_ATTEMPTS; attempt += 1) {
    const smoke = await read()
    const state = liveSmokeState(smoke, deploymentTarget, smokeInput)
    if (state === "ready") return smoke
    if (state === "failed" || attempt === SMOKE_SETTLE_ATTEMPTS) throw fail("smoke_failed", "smoke")
    await settleDelay()
  }
  throw fail("smoke_failed", "smoke")
}
...
function errorResult(operationId, lane, checks, error, identifiers) {
  ...
  const failedName = stage === "approval" ? "approval" : stage === "candidate" ? "candidate" : stage === "remote" ? "remote_heads" : stage === "mapping_push" ? "mapping_branch" : stage === "pr" ? "mapping_pr" : stage === "ci" ? "mapping_ci" : stage === "merge" ? "mapping_merge" : stage === "map" ? "map_readback" : stage === "candidate_commit" ? "site_candidate" : stage === "push" ? "gh_pages_push" : stage === "dispatch" ? "dispatch" : stage === "workflow" ? "deployment_run" : stage === "pages" ? "pages" : stage === "smoke" ? "smoke" : "provider"
...
function candidateCommitTreeMatches(value, candidateSha, expectedFiles, expectedSiteSha) {
  if (!isRecord(value) || value.candidate_sha !== candidateSha || !Array.isArray(value.files)) throw fail("remote_drift", "candidate_commit")
  const expected = new Map(expectedFiles.map(({ relative, bytes }) => [`site/${relative}`, bytes]))
  const seen = new Set()
  const actual = []
  ...
  for (const entry of value.files) {
    if (!isRecord(entry) || typeof entry.relative !== "string" || entry.mode !== "100644" || entry.type !== "blob"
      || !Buffer.isBuffer(entry.bytes) || seen.has(entry.relative) || !entry.relative.startsWith("site/") || entry.relative === "site/") {
      throw fail("remote_drift", "candidate_commit")
    }
    seen.add(entry.relative)
    const expectedBytes = expected.get(entry.relative)
    if (expectedBytes === undefined || !sameBytes(entry.bytes, expectedBytes)) matches = false
    actual.push({ relative: entry.relative.slice("site/".length), bytes: entry.bytes })
  }
  actual.sort((left, right) => utf8Order(left.relative, right.relative))
  return matches && seen.size === expected.size && candidateTreeSha256(actual) === expectedSiteSha
}
...
function validatePages(value, runId, siteCommit) {
  ...
  return { deployment_id: deploymentId, run_id: runId, site_commit: siteCommit, status: "success", url: url.href }
}
...
export async function routinePublicationHandoff(approvedOperation, seams) {
  ...
  try {
    ...
    if (!sameBytes(initial.map_bytes, operation.proposed_site_content_bytes)) {
      ...
      }, "mapping_push"), operation, operation.proposed_site_content_bytes)
    ...
    const deployedCommit = await call(localGit, "readCandidateCommit", {
      candidate_sha: operation.approval.expected_gh_pages_sha,
    }, "candidate_commit")
    if (candidateCommitTreeMatches(
      deployedCommit,
      operation.approval.expected_gh_pages_sha,
      candidate.files,
      operation.candidate_identity.site_sha256,
    )) {
      identifiers.site_commit = operation.approval.expected_gh_pages_sha
      checks.push(check("site_convergence"))
      const converged = result(operationId, lane, "no_change", "公開網站已是核准版本。", checks, "none", null, identifiers)
      converged.convergence = {
        exact: true,
        desired_site_sha256: operation.candidate_identity.site_sha256,
        public_site_sha256: operation.candidate_identity.site_sha256,
        live_site_sha256: operation.candidate_identity.site_sha256,
        provider_site_commit: operation.approval.expected_gh_pages_sha,
      }
      return converged
    }
    ...
    await waitForLiveSmoke(
      async () => await call(provider, "anonymousSmoke", smokeInput, "smoke"),
      deploymentTarget,
      smokeInput,
    )
