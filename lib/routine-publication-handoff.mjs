...
export async function routinePublicationHandoff(approvedOperation, seams) {
  ...
  try {
    ...
    if (!sameBytes(initial.map_bytes, operation.proposed_site_content_bytes)) {
      ...
      await call(provider, "readRequiredCi", { head_sha: mappingBranch.head_sha, workflow: MAPPING_WORKFLOW, job: MAPPING_JOB }, "ci").then((value) => validateCi(value, mappingBranch.head_sha))
    ...
    const newIds = setDifference(afterIds, beforeIds)
    if (newIds.length !== 1) {
    ...
    await call(provider, "readDeploymentRun", { id: runId, site_commit: siteCommit, publication_mode: DEPLOY_MODE, workflow: DEPLOY_WORKFLOW, ref: DEPLOY_REF, head_sha: currentMainSha }, "workflow").then((value) => validateRun(value, runId, siteCommit, runName, currentMainSha))
    checks.push(check("deployment_run"))
    const pages = await call(provider, "readPagesDeployment", { run_id: runId, site_commit: siteCommit }, "pages")
