...
**Status:** Renderer, local product, sealed-release custody, and T11 handoff are complete. T12 repository-only Wave C implements the approved dedicated `gh-pages` candidate/verifier and thin exact-SHA Pages workflow; live repository visibility, Pages controls, deployment approval, provider readback, and browser QA remain gated. Hermes T13 remains start-gated.
...
Canonical Drive Vault Markdown (read-only)
  ...
  → atomic promotion of a validated sealed release
  → exact-byte projection into an isolated dedicated `gh-pages` candidate
  → manual exact-SHA workflow validation and GitHub environment approval
  → provider deployment/readback and anonymous browser QA
```
...
20. As a site administrator, I want release evidence and source/output hashes outside the served Pages site, with only the bounded redacted `.publication/` metadata retained on the publication branch, so that releases are auditable without leaking local paths or runtime data.
...
- The exact Pages delivery mechanism is locked by `config/github-pages-deployment-contract-v1.json`: source/tools/workflow remain on `main`; approved generated bytes are projected to `refs/heads/gh-pages` as root `site/` plus bounded `.publication/` metadata; the workflow uploads only verifier-staged `site/`. The project-site base path is `/Tyler-Vault_PaperNote_ReadingSite`; GitHub owns Pages artifact retention and provider deployment state. No custom rollback/claim authority is introduced. The local prepare CLI requires exact `--source-root`; the operator-supplied canonical source-tree root is trusted authority for source custody, candidate output must be disjoint from it, and the source-root path is not emitted into candidate metadata.
- Candidate preparation/staging cleanup is a best-effort non-recursive `rmdir` after immediate no-link and dev/ino checks. Its parent and output staging root are trusted against cooperating same-user replacement during cleanup; it does not claim resistance to a compromised/cooperating same-user writer or absolute atomic replacement safety.
...
- The existing private repository and remote `eyeyesight/Tyler-Vault_PaperNote_ReadingSite` are approved. The confirmed target remains that repository made public plus free GitHub Pages; repository visibility, Pages settings, the exact first deployment, and later naming/access-policy mutations remain explicit gated operations.
- Responsibility is split by plane: the project owns public-content audit and local release/candidate binding; GitHub owns collaborators, rulesets, Actions permissions, environment review, and Pages settings; GitHub Actions/Pages owns workflow-run, artifact, deployment/status IDs, concurrency, and provider URL.
- The normative machine contract is `config/github-pages-deployment-contract-v1.json` with its schema. Candidate metadata is `.publication/gh-pages-candidate-v1.json` under the dedicated `refs/heads/gh-pages`; it binds the verified sealed release identity, exact source artifact inventory/digest, approved manifest/receipt and rights authority, expected URL/base path, and the exact `site/` inventory/digest. The finalized one-time audit is `.publication/github-launch-audit-v1.json`. Neither metadata file is uploaded to the served Pages artifact.
- Generated HTML/CSS/JavaScript/search/graph/static assets exist in Git only under `gh-pages/site/`, never on `main`. The publication commit tree is exactly `site/` plus `.publication/`; branch preparation uses an isolated worktree or equivalent isolated index, never a shared dirty source worktree.
- The default-branch `.github/workflows/deploy-pages.yml` is manual-only (`workflow_dispatch`) and accepts exactly three inputs: lowercase 40-hex `site_commit`, lowercase 64-hex `candidate_digest`, and lowercase 64-hex `launch_audit_digest`. Before candidate use it requires both `refs/heads/main` and repository default branch `main`. Its validation job checks out the exact workflow source, proves the requested commit is contained in `refs/heads/gh-pages`, compares branch metadata with both GitHub-authenticated expected digests, verifies the finalized launch audit, and stages only exact `site/` bytes into a fresh child directory. It does not build Quartz or mutate the candidate.
- The deploy job starts only after validation and enters the protected `github-pages` environment. Arke prepares/pushes and dispatches the exact workflow run in the operator role through the shared GitHub account `eyeyesight`; Tyler records the separate visibility approval before public mutation, then after validation and exact digest comparison reviews and approves the specific GitHub environment deployment in the approver role. GitHub records both roles as `eyeyesight`, not as independently verifiable accounts. Tyler correlates `workflow_run_id`, `run_started_at`, `environment_reviewed_at`, and the Telegram decision reference with the exact SHA and digest summary; time/Telegram evidence is operational correlation, not cryptographic identity proof. Self-review prevention is disabled because the required reviewer uses the same account, while the required environment approval remains. Telegram/local content approval and repository visibility approval cannot substitute for this provider-owned deployment approval. If the account is compromised, it can initiate and approve both human events; exact bytes, digests, provider readback, and QA controls remain unchanged.
- The launch-audit lifecycle is strictly ordered: previsibility audit → visibility approval → post-visibility repository/API/anonymous-repository readback → finalize `audit_digest`. It uses one authenticated complete-pagination GitHub API machine lane; UI is optional corroboration. Known clones or cached views remain `not-provable` with `zero_gate: false`.
- Public exposure is blocked until the pinned-stack security gate, integrated local rehearsal, authorized sealed-release handoff, exact candidate verification, finalized launch-audit/visibility gate, remote control-plane readback, and both distinct human approvals pass. Repository-only tests cannot claim live success.
- Provider success must be read back by workflow/deployment/status/environment URL IDs before independent anonymous HTTP/browser QA. Unknown provider outcomes are reconciled against the same run/deployment IDs; the workflow does not mint a custom claim, retry, or rollback authority.
- Direct GitHub edits to generated HTML invalidate the sealed manifest/digests and are not deployable until the source change is written back, rebuilt/resealed, and revalidated. New pages, slugs, search, or graph changes always return to source build.
- A private repository must never be presented as equivalent to a private website. PWA/offline installation, custom domain, analytics, comments, access-controlled hosting, and Zotero-triggered automatic Pages deployment are deferred.
...
2. **Safety lifecycle test:** start with a last-known-good site, inject expired/digest/hash/path/secret/link failures, and prove no public artifact changes; then pass a valid sealed release, project it to an exact candidate, and prove the deployment/readback path without treating a custom claim as authority. The launch-audit digest is finalized after sealed artifact creation plus visibility readback and before workflow dispatch/deployment approval; Pages post-deploy QA runs only after provider success.
...
- Public deployment before the pinned-stack security resolution, integrated local rehearsal, authorized sealed-release handoff, exact dedicated-branch candidate verification, finalized launch-audit/visibility gate, remote control-plane readback, and `github-pages` environment approval pass. Post-deploy provider readback and anonymous browser QA remain separate success-after-provider steps. The hosting target itself is already approved as a public GitHub repository plus free GitHub Pages.
...
- Prototype evidence and reports remain in a local Hermes prototype workspace outside this repository; they are not production dependencies or checkpoint artifacts.
