...
function tracerQuartzConfig(source) {
  return source
    .replace("pageTitle: Quartz 5", "pageTitle: Manifest Quartz Tracer")
    .replace("enableSPA: true", "enableSPA: false")
    .replace("enablePopovers: true", "enablePopovers: false")
    .replace("provider: plausible", "provider: null")
    .replace("baseUrl: quartz.jzhao.xyz", "baseUrl: example.invalid")
    .replace("fontOrigin: googleFonts", "fontOrigin: local")
    .replace(/(source: "@quartz-community\/(?:alias-redirects|content-index|og-image|cname|canvas-page|tag-page|explorer|graph|search|breadcrumbs)"\n\s+enabled:) true/g, "$1 false")
    .replace(/(source: "@quartz-community\/footer"\n\s+enabled:) true/g, "$1 false")
}
...
function cssUrls(css) {
  const urls = []
  for (const match of css.matchAll(/url\(\s*(?:(['"])([\s\S]*?)\1|([^)]*?))\s*\)/gi)) urls.push((match[2] ?? match[3] ?? "").trim())
  return urls
}
...
function validateCandidateUrl(raw, baseRoute, approvedRoutes, expectedAssets, assetContext) {
  ...
  if (/^(?:javascript|vbscript|data|file)\s*:/i.test(value.replace(/[\u0000-\u0020]+/g, ""))) {
    throw new TracerError("CANDIDATE_UNSAFE_SCHEME", "candidate contains an unsafe URL scheme")
  }
...
/** @param {string} candidate @param {string} run @param {Map<string,{route:string}>} records @param {Set<string>} suppressedTargets @param {string[]} privatePaths @param {ReadonlyArray<{relative:string,fileClass:string,sha256:string}>} baseline */
async function gateCandidate(candidate, run, records, suppressedTargets, privatePaths, baseline) {
  ...
  for (const file of files) {
    ...
    if (!/\.(?:css|js|map)$/i.test(file.relative) && /(?:javascript|vbscript|data|file)\s*:/i.test(text)) throw new TracerError("CANDIDATE_UNSAFE_SCHEME", "candidate contains an unsafe URL scheme", { file: file.relative })
...
async function build(safe) {
  ...
  try {
    ...
    const defaultConfig = await readFile(path.join(safe.installedRoot, "quartz.config.default.yaml"), "utf8")
    await Promise.all([
    ...
    await writeFile(path.join(toolchain, "quartz.config.yaml"), tracerQuartzConfig(defaultConfig), { flag: "wx" })
    ...
    // This frozen manifest is captured before every test hook and before the
    // only sanctioned mutation (closed-set virtual-page pruning).
